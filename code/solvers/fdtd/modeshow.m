...
emax = max(abs(Ez_src));
Ez_src = Ez_src./max(Ez_src);
Hx_src = Hx_src./max(Hx_src);
...
f = 1i*2*pi*frequency;
Nx = N(1); Ny = N(2);
N_lambda=1/(frequency);
%% Calculation loop
for T=1:12000
    % Curl Ex
    Ez(2.5/dy,21)=sin(((2*pi*(1/(N_lambda))*(T)*dt)));
   ...
   % CEx(nx1_src:nx2_src,ny_src-1) = CEx(nx1_src:nx2_src,ny_src-1) - ezsrc;
  ...
  %  CHz(nx1_src:nx2_src,ny_src) = CHz(nx1_src:nx2_src,ny_src) - hxsrc;
...
%     eps_space = assign_space(eps_space, xrange, yrange, modLeft, (eps_wg + 0.1*eps_wg*cos(Omega * T * dt))); 
%     eps_space = assign_space(eps_space, xrange, yrange, modRight, (eps_wg + 0.1*eps_wg*cos(Omega * T * dt + pi/2))); 
...
figure(2); hold on;
title('Structure of waveguide');
contourf(eps_space); hold off;