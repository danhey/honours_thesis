...
yrange = [0 23];            % y boundaries in L0
...
%        REF(:,i) = REF(:,i) + (K(i)^T)*Ez(:,22/dy)*dt;
%        DET(:,i) = DET(:,i) + (K(i)^T)*Ez(:,ny_src+1)*dt;
%     end
end

%Coherence length amplitudes
for i = 1:Ny
mode_1_test(i) = sum(abs(mode_1(:,i)));
mode_2_test(i) = sum(abs(mode_2(:,i)));
end

mode_1_test = mode_1_test';
mode_2_test = mode_2_test';

figure(2);
hold on
plot(1:Ny,mode_1_test);
plot(1:Ny,mode_2_test);
hold off
