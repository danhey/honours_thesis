function [Ez, Hx, Hy, omega, A] = solve_frequency(L0, wvlen0, Omega, Nsb, xrange, yrange, eps_r, mod_reg, mod_phi, Jz0, Npml)
%% This function performs an implementation of the MF-FDFD method described in section 4.3.2. The inputs are the normalisation constant L0, initial wavelength wvlen0, modulation frequency Omega, matrices containing the x and y range of the simulation, and the permittivity and modulation matrices (eps_r, mod_reg)
...
vector_eps = reshape(e0 * eps_r, M, 1); vec_phi = reshape(mod_phi, M, 1); 
% Create T diags
T_eps = spdiags(vector_eps, 0, M, M); T_phi = spdiags(exp(1i*vec_phi), 0, M, M);
...
for i = 1 : (2*Nsb+1)
    ...
    Hx{i} = -1/(1i*omega(i)) * u0^-1 * Syf{i}*Dyf * Ez{i}; 
    Hy{i} = 1/(1i*omega(i)) * u0^-1 * Sxf{i}*Dxf * Ez{i}; 
