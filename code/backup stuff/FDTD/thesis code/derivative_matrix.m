function [Dx, Dy, Dz] = derivative_matrix(w, s, dL, N)
% w: one of 'x', 'y', 'z'
% s: one of 'f' and 'b'
...
switch w 
    case 'x'
        ...
        for n = 0 : 1 : (Ny*Nz - 1)
            Dx(1 + n*Nx : (n+1)*Nx, 1 + n*Nx : (n+1)*Nx) = 1/dw * block_x; 
        end
...
%         block_y = zeros(Nx*Ny); 
        ...
        for n = 0 : 1 : Nz-1
            Dy(1 + n*(Nx*Ny) : (n+1)*(Nx*Ny) , 1 + n*(Nx*Ny) : (n+1)*(Nx*Ny)) = 1/dw * block_y; 
        end
        
    case 'z'
        % Construct Dzs by stacking identity matrices
        for n = 0 : 1 : Nz - 1
            Dz(1 + n*(Nx*Ny) : (n+1)*(Nx*Ny) , 1 + n*(Nx*Ny) : (n+1)*(Nx*Ny)) = -speye(Nx*Ny); 

            if n < Nz - 1
                Dz(1 + n*(Nx*Ny) : (n+1)*(Nx*Ny) , 1 + (n+1)*(Nx*Ny) : (n+2)*(Nx*Ny)) = speye(Nx*Ny); 
            else
                Dz(1 + n*(Nx*Ny) : (n+1)*(Nx*Ny) , 1 : Nx*Ny) = speye(Nx*Ny); 
            end
        end
        % Rescale Dws = Dzs
        Dz = 1/dw * Dz; 
